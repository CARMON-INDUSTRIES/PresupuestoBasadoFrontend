...
const form = ref({ recursoPropio: '', recursoEstatal: '', recursoFederal: '' })
const { guardarDatos } = useFormularioPersistente({
  form,
  storageKey: 'FormularioRamo',
  load: async () => null,
  save: async (data) => (await api.post('/Ramo', data)).data,
})
async function submitForm() {
  ...
  try {
    ...
    await router.push('/formulario-clasificacion')
  } catch (error) {
    Notify.create({
      type: 'negative',
